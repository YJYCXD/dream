...
void ContactSave(SC* ps)
{
	...
	for (i = 0; i < ps->size; i++)
	{
		fwrite(&(ps->human[i]), sizeof(struct PeopleInfo), 1, pf);//存储的大小是PeopleInfo的大小，不是SC的大小
	}
